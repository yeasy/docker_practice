...
我们可以使用 `AS` 来为某一阶段命名（关键字大小写要与 `FROM` 一致，否则 `docker buildx build --check` 的 [FromAsCasing](https://docs.docker.com/reference/build-checks/from-as-casing/) 会报 `'as' and 'FROM' keywords' casing do not match`），例如
