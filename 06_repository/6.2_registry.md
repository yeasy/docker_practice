...
> **注意**：这样启动的 registry **没有任何认证、也没有 TLS**，因此这里刻意绑定到
> `127.0.0.1`，仅供本机实验。不要直接把它绑到 `0.0.0.0` 或内网地址——那等于允许同网段
> 任意主机匿名推送镜像（镜像投毒：覆盖你的 `latest` 标签，下游全部拉到攻击者的镜像）。
> 需要多机访问请直接用 [6.3 节](6.3_registry_auth.md) 的 TLS + 认证方案。
...
> **警告**：下面的 `insecure-registries` 会关闭该地址的 TLS 校验，推送和拉取都是明文、可被中间人篡改；再叠加上 registry 默认无认证，等于把「任意主机可匿名投毒」的仓库开在内网。**仅限隔离的实验网络**，生产一律走 [6.3 节](6.3_registry_auth.md) 的 TLS + 认证。
