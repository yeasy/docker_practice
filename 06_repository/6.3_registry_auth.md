...
> **版本说明**：使用 `httpd:2.4-alpine` 基于 Apache 2.4 的精简镜像。如需其他版本，请指定具体版本号如 `httpd:2.4.58-alpine`；不要用 `latest`（本节下文与 7 章都要求固定版本以保证可重复性）。
...
> **版本说明**：示例使用 `ubuntu:24.04`，这是仍受 Docker 支持的 LTS 版本（当前最新 LTS 为 26.04）。推送到私有仓库时保持了原有的版本标签，建议生产环境明确指定镜像版本号。
如果我们退出登录，尝试推送镜像。
