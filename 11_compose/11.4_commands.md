...
* `--verbose` 输出更多调试信息。（Compose V2 中该选项仍然生效，但已从 `--help` 中隐藏，不再是推荐用法；请改用 `docker --log-level debug compose ...`。）
...
服务容器一旦构建后，将会带上一个标记名，例如对于 web 项目中的一个 db 服务，构建出的镜像是 `web-db`。（Compose V1 用下划线拼接为 `web_db`，V2 起统一改为连字符，只有加 `--compatibility` 才会回到下划线。）
