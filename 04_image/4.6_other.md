...
除了标准的使用 `Dockerfile` 生成镜像的方法外，由于各种特殊需求和历史原因，还提供了一些其他方法用以生成镜像。
...
> **版本提示**：`alpine:latest` 为最新版本的 Alpine Linux。如果需要特定版本号（如 `alpine:3.24`），可以明确指定以确保可重现性；注意选一个仍在维护的版本，`alpine:3.20` 已于 2026-04 结束支持。
保存镜像的命令为：
